...
def test_research_requires_authenticated_owner_even_on_loopback(tmp_path: Path) -> None:
    ...
    unconfigured = CommandCenterService(
        CommandCenterConfig(queue_path=tmp_path / "unconfigured.jsonl")
    )
    unconfigured_status, _ = unconfigured.handle_command(
        "research NIFTY breadth",
        owner_authenticated=True,
    )
    assert unconfigured_status == HTTPStatus.UNAUTHORIZED
    assert unconfigured.queue.count == 0

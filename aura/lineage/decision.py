...
class DecisionLineageRecord(BaseModel):
    ...
    @classmethod
    def build(cls, *, context: AgentContext, round_result: AgentRound, memo: CEODecisionMemo,
              data_quality: DataQualityReport | None, agent_policy: AgentPolicyDecision | None,
              deliberation: DeliberationMemo | None) -> "DecisionLineageRecord":
        ...
        sources = tuple(source for evidence in round_result.evidence for source in evidence.sources)
        ...
        latest_source_observed_at = max((source.observed_at for source in sources), default=None)
        component_hashes = {
            "candle_series_hash": _hash([candle.model_dump(mode="json") for candle in context.candles]),
            "metadata_hash": _hash(context.metadata),
            "evidence_hash": _hash([item.model_dump(mode="json") for item in round_result.evidence]),
            "failures_hash": _hash([item.model_dump(mode="json") for item in round_result.failures]),
            "quality_hash": _hash(data_quality),
            "deliberation_hash": _hash(deliberation),
            "ceo_hash": _hash(memo.model_dump(mode="json")),
            "agent_policy_hash": _hash(agent_policy.model_dump(mode="json") if agent_policy is not None else None),
        }
        lineage_payload = {
            "lineage_version": "decision-lineage-v1", "correlation_id": context.correlation_id,
            "symbol": context.symbol, "timeframe": context.decision_timeframe,
            "decision_time": context.created_at, "candle_count": len(context.candles),
            **component_hashes, "source_ids": source_ids,
            "latest_source_observed_at": latest_source_observed_at,
        }
        return cls(**lineage_payload, lineage_hash=_hash(lineage_payload))

    def verify_hash(self) -> bool:
        return self.lineage_hash == _hash(self._payload())

    def verify(self, *, context: AgentContext, round_result: AgentRound, memo: CEODecisionMemo,
               data_quality: DataQualityReport | None, agent_policy: AgentPolicyDecision | None,
               deliberation: DeliberationMemo | None) -> bool:
        if not self.verify_hash():
            return False
        rebuilt = type(self).build(context=context, round_result=round_result, memo=memo,
                                   data_quality=data_quality, agent_policy=agent_policy,
                                   deliberation=deliberation)
        return self == rebuilt

    def _payload(self) -> dict[str, Any]:
        return {
            "lineage_version": self.lineage_version, "correlation_id": self.correlation_id,
            "symbol": self.symbol, "timeframe": self.timeframe, "decision_time": self.decision_time,
            "candle_count": self.candle_count, "candle_series_hash": self.candle_series_hash,
            "metadata_hash": self.metadata_hash, "evidence_hash": self.evidence_hash,
            "failures_hash": self.failures_hash, "quality_hash": self.quality_hash,
            "deliberation_hash": self.deliberation_hash, "ceo_hash": self.ceo_hash,
            "agent_policy_hash": self.agent_policy_hash, "source_ids": self.source_ids,
            "latest_source_observed_at": self.latest_source_observed_at,
        }


def _hash(value: Any) -> str:
    canonical = json.dumps(_canonicalize(value), separators=(",", ":"), sort_keys=True,
...
def _canonicalize(value: Any) -> Any:
    ...
    if isinstance(value, (set, frozenset)):
        ...
        return sorted(normalized, key=lambda item: json.dumps(item, separators=(",", ":"), sort_keys=True, ensure_ascii=False))

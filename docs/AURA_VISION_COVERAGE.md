...
| Multi-model local/provider AI council | IMPLEMENTED | `aura/agents/ai_council.py`, `aura/agents/ollama_provider.py`, `aura/agents/openai_provider.py`, `aura/agents/adaptive_model_router.py` | Ollama models require local installation; OpenAI models require an operator-owned API key. |
...
| Controlled repair/monitor/code-update AI | IMPLEMENTED, OWNER-GATED | `aura/maintenance/`, `aura/ai/openai_responses.py`, `tests/test_maintenance_change_control.py` | AI proposes and sandbox-validates; exact owner approval is required before a development worktree changes. No automatic merge/deploy. |
